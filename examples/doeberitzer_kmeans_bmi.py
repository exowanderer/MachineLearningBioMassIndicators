...
if __name__ == "__main__":
    ...
    info_message("Generate Sentinel KMeansBMI Instance")
    scene_data = KMeansBMI(
    ...
    if clargs.ndvi:
        ...
        scene_data.compute_spatial_kmeans(bmi='ndvi')

        info_message(
            "Computing temporal K-Means for each scene NDVIs over time"
        )
        scene_data.compute_temporal_kmeans(bmi='ndvi')
    ...
    if clargs.gci:
        ...
        scene_data.compute_spatial_kmeans(bmi='gci')

        info_message(
            "Computing temporal K-Means for each scene GCIs over time")
        scene_data.compute_temporal_kmeans(bmi='gci')
    ...
    if clargs.rci:
        ...
        scene_data.compute_spatial_kmeans(bmi='rci')

        info_message(
            "Computing temporal K-Means for each scene RCIs over time")
        scene_data.compute_temporal_kmeans(bmi='rci')
