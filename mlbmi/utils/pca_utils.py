...
def sanity_check_spatial_pca(pca, image, quantile_range=(1, 99),
    ...
    components_pred = pca.predict(pixel_scaled)
    ...
    for k in range(pca.n_components):
        axs[k + 1].imshow(
            (components_pred == k).reshape(image.shape),
            interpolation='None'
        )
...
def sanity_check_temporal_pca(
    ...
    # Predict each components value per pixel
    components_pred = pca.predict(samples_scaled)
    ...
    # Cycle through and plot each components_pred image per 'class'
    for k in range(pca.n_components):
        axs[k + 2].imshow((components_image == (k + 1)), interpolation='None')
